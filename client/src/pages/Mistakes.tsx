...
import { isCleanLossTag } from "@/lib/cleanLoss";
...
export default function Mistakes() {
  ...
  const mistakeStats = useMemo(() => {
    return calculateMistakeStats(filteredTrades)
      .filter((m) => !isCleanLossTag(m.mistake))
  ...
  const thisWeekMistakeTrades = thisWeekTrades.filter((t) => t.mistakes?.some((m) => !isCleanLossTag(m)));
  const thisWeekMistakeR = thisWeekMistakeTrades.reduce((s, t) => s + (t.realisedR || 0), 0);

  // ── Summary metrics ──
  const totalMistakeR = filteredTrades
    .filter((t) => t.mistakes?.some((m) => !isCleanLossTag(m)))
    .reduce((sum, t) => sum + (t.realisedR || 0), 0);
  const totalMistakenTrades = filteredTrades.filter((t) => t.mistakes?.some((m) => !isCleanLossTag(m))).length;
  const cleanLossTrades = filteredTrades.filter(
    (t) => (t.realisedR || 0) < 0 && (!t.mistakes?.length || t.mistakes.every((m) => isCleanLossTag(m)))
  ).length;
  ...
  const monthlyTrend = useMemo(() => {
    ...
    filteredTrades.forEach((t) => {
      ...
      map[key].total++;
      if (t.mistakes?.some((m) => !isCleanLossTag(m))) map[key].mistakes++;
    });
  ...
  const monthlyByMistakes = useMemo(() => {
    ...
    filteredTrades.forEach((t) => {
      ...
      map[key].totalTrades++;
      if (t.mistakes?.some((m) => !isCleanLossTag(m))) {
